...
const mixRgb = (from, to, amount) => ({ r: Math.round(from.r + (to.r - from.r) * amount), g: Math.round(from.g + (to.g - from.g) * amount), b: Math.round(from.b + (to.b - from.b) * amount) })
...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const ensureRenderLoop = () => { if (animationFrame === null) animationFrame = window.requestAnimationFrame(render) }
    ...
    const sampleText = async () => {
      ...
      if (reducedMotion) {
        particles.forEach((particle) => { particle.x = particle.targetX; particle.y = particle.targetY; particle.startX = particle.targetX; particle.startY = particle.targetY; particle.delay = 0 })
        gathering = false
      } else {
        startGather(false)
      }
    ...
    const handlePointerEnter = (event) => { handlePointerMove(event); if (trigger === 'hover') startGather(true) }
